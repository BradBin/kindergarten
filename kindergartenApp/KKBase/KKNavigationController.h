 ...
 * 使用说明：
 *
 *  1、将`KKNavigationController.{h/m}`文件拖入项目
 *
 *  2、只需将导航栏类设置为`KKNavigationController`即可
 *
 ...
 *     如果是个别页面有定制的导航栏，控制器也可以通过重写`-kk_navigationControllerClass`方法返回对应的导航栏
 */
...
/// 仅限于内部使用
@interface _KKContainerNavigationController : UINavigationController
@end


/// 导航栏控制器
@interface KKNavigationController : _KKContainerNavigationController
...
/**
 返回控制器的导航栏,默认`KKContainerNavigationController.class`
